...
func (awsSvc defaultAwsService) stateStoreBucketName(region, dns string) string {
...
func (awsSvc defaultAwsService) stateStoreBucketExist(region, dns string) bool {
	sess := awsSvc.awsSession(region)
	s3Svc := s3.New(sess)
...
func (awsSvc defaultAwsService) ClusterExist(config config.ClusterConfig) bool {
	...
	stateStoreBucketName := awsSvc.stateStoreBucketName(config.Region, config.DnsZone)
